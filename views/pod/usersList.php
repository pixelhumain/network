			...
			<?php }
				...
				foreach ($users as $e) { 
					...
					?>
						...
						<a href="javascript:;" onclick="getAjaxFiche('#element.detail.type.<?php echo $e["type"] ?>.id.<?php echo $e["id"] ?>', true);" title="<?php echo $name ?>" class="btn no-padding contentImg <?php echo $grayscale ?>">
		...
		<?php
			...
			if((@$countStrongLinks && $countStrongLinks != 0) || (@$countLowLinks && $countLowLinks != 0)){
				...
				if (@$countStrongLinks && !empty($countStrongLinks)){
					...
					echo "<div class='col-md-4 inline' style='".$style."'>".
